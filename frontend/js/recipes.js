...
function renderRecipes() {
  ...
  grid.innerHTML = list.map(r => `
    <article class="recipe-card" data-id="${r.id}">
      ...
      <div class="recipe-body">
        <h3>${r.name}</h3>
        <p>${r.description || "Ricetta personale pronta da aggiungere alla lista della spesa."}</p>
  ...
  `).join("");
  grid.querySelectorAll(".recipe-card").forEach(card => card.addEventListener("click", () => openDetail(Number(card.dataset.id))));
}
...
async function renderDaily() {
  ...
  card.innerHTML = `
    ...
    <div class="daily-actions">
      <button class="primary-btn" id="add-daily">Aggiungi trovati</button>
      <button class="ghost-btn" id="daily-info">Dettagli</button>
    </div>
  `;
...
function openBuilder() {
  selectedIngredients = [];
  activeRecipe = null;
  document.getElementById("builder-title").textContent = "Nuova ricetta";
...
function closeBuilder() { document.getElementById("recipe-builder").classList.add("hidden"); }
...
function searchProducts() {
  ...
  if (!q) { box.innerHTML = `<div class="empty-state">Cerca un prodotto da usare come ingrediente.</div>`; return; }
  const found = products.filter(p => p.name.toLowerCase().includes(q) || (p.category || "").toLowerCase().includes(q)).slice(0, 20);
  box.innerHTML = found.length ? found.map(p => `
    <div class="product-result">
      <img src="${img(p.image)}" onerror="this.src='/static/images/placeholder.jpg'">
      <div><b>${p.name}</b><small>${storeName(p)} · ${p.unit || "pz"} · ${euro(currentPrice(p))}</small></div>
      <button data-id="${p.id}">+</button>
    </div>
  `).join("") : `<div class="empty-state">Nessun prodotto trovato.</div>`;
  box.querySelectorAll("button[data-id]").forEach(btn => btn.addEventListener("click", () => addIngredient(Number(btn.dataset.id))));
}
...
function renderSelectedIngredients() {
  ...
  if (!selectedIngredients.length) {
    box.innerHTML = `<div class="empty-state">Aggiungi prodotti dal catalogo.</div>`;
  } else {
    box.innerHTML = selectedIngredients.map((i, idx) => `
      <div class="ingredient-row">
        ...
        <div><b>${i.product.name}</b><small>${storeName(i.product)} · ${euro(currentPrice(i.product))}</small></div>
        ...
        <div class="ingredient-controls">
          ...
          <input data-field="amount_unit" data-idx="${idx}" placeholder="g, ml, pz..." value="${i.amount_unit || ""}">
  ...
  document.getElementById("builder-total").textContent = euro(selectedIngredients.reduce((s,i) => s + currentPrice(i.product) * Number(i.cart_quantity || 1), 0));
  box.querySelectorAll("[data-remove]").forEach(btn => btn.addEventListener("click", () => { selectedIngredients.splice(Number(btn.dataset.remove), 1); renderSelectedIngredients(); }));
  box.querySelectorAll("input[data-field]").forEach(inp => inp.addEventListener("input", () => {
  ...
  content.innerHTML = `
    <div class="detail-header">
      ...
      <div>
        ...
        <h2>${r.name}</h2>
        <p>${r.description || "Seleziona cosa aggiungere e regola le quantità prima di creare la lista."}</p>
    ...
    <div class="ingredient-checks">
      ${r.items.map(it => `
        <label class="check-row">
          ...
          <div><b>${it.product?.name || "Prodotto"}</b><small>${it.amount || ""} ${it.amount_unit || ""} · ${it.product?.supermarket_name || ""} · ${euro(it.product?.current_price || 0)}</small>${(it.cheaper_alternatives || []).length ? `<div class="alt-box">Risparmio possibile: ${it.cheaper_alternatives[0].product.name} (${euro(it.cheaper_alternatives[0].product.current_price)})</div>` : ""}</div>
          <input type="number" min="1" value="${it.cart_quantity || 1}" data-qty="${it.id}">
        </label>`).join("")}
    </div>
    <div class="detail-footer"><strong>Totale stimato: ${euro(r.estimated_total)}</strong><button class="primary-btn" id="add-recipe-cart">Aggiungi selezionati alla lista</button></div>
  `;
