...
def patch_main_py(project_root: Path) -> None:
    ...
    backup = main_path.with_suffix(".py.bak_profile_recipes_v8")
...
def patch_models_py(project_root: Path) -> None:
    ...
    backup = models_path.with_suffix(".py.bak_profile_recipes_v8")
    if not backup.exists():
...
def main() -> None:
    ...
    if not (project_root / "app").exists() or not (project_root / "frontend").exists():
        raise RuntimeError("Estrai lo ZIP nella root del progetto, poi esegui python install_profile_recipes_v8.py")
    ...
    print("\nPatch Profilo + Ricette v8 installata.")
    print("Riavvia uvicorn e fai Ctrl+F5 nel browser.")
    print("La ricetta del giorno ora è locale: non usa siti esterni.")
