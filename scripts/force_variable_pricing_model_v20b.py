...
def main() -> None:
    ...
    text = patch_class(text, "Products", PRODUCT_FIELDS, [
        "price_unit = Column",
        "price_type = Column",
    ...
    text = patch_class(text, "Cart", CART_FIELDS, [
        "manual_price = Column",
        "checked = Column",
        "owner_id = Column",
        "quantity = Column",
    ])
    text = patch_class(text, "ShoppingHistoryItem", HISTORY_ITEM_FIELDS, [
        "manual_price = Column",
        "final_price_paid = Column",
        "protein = Column",
        "supermarket_name = Column",
        "quantity = Column",
    ])

    if text != original:
        backup = path.with_suffix(".py.bak_variable_pricing_v20b")
    ...
    products = class_block(updated, "Products")
    cart = class_block(updated, "Cart")
    history_items = class_block(updated, "ShoppingHistoryItem")

    checks = {
        "Products.price_type": "price_type = Column" in products,
        "Products.price_unit": "price_unit = Column" in products,
        "Cart.estimated_weight": "estimated_weight = Column" in cart,
        "Cart.actual_weight": "actual_weight = Column" in cart,
        "Cart.manual_price": "manual_price = Column" in cart,
        "ShoppingHistoryItem.weight_bought": "weight_bought = Column" in history_items,
        "ShoppingHistoryItem.final_price_paid": "final_price_paid = Column" in history_items,
    }
    ...
    print("Poi controlla in Swagger: GET /admin/debug/products-model")
    print("Deve dare has_price_type=true e db_has_price_type=true.")
