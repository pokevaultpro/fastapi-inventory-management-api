...
PATCH_FILES = [
    ...
    "docs/ADMIN_RECIPES_HISTORY_V11.md",
]
...
def patch_main_py(project_root: Path) -> None:
    ...
    backup = main_path.with_suffix(".py.bak_admin_recipes_history_v11")
    ...
    text = ensure_import(text, "from app.routers import profile, recipe_smart, flyer_catalog, admin")
...
def insert_after(text: str, anchor: str, addition: str) -> str:
    first = addition.strip().splitlines()[0].strip()
    if first in text:
        return text
...
def patch_models_py(project_root: Path) -> None:
    ...
    backup = models_path.with_suffix(".py.bak_admin_recipes_history_v11")
    if not backup.exists():
...
def main() -> None:
    ...
    if not (project_root / "app").exists() or not (project_root / "frontend").exists():
        raise RuntimeError("Estrai lo ZIP nella root del progetto, poi esegui python install_admin_recipes_history_v11.py")
    ...
    print("\nPatch v11 installata.")
    print("Ora esegui anche la migrazione DB se non l'hai già fatta, poi riavvia uvicorn e fai Ctrl+F5.")
    print("Admin page: frontend/admin.html oppure /admin.html sul dominio.")
